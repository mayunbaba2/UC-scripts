...
USL.__defineSetter__("disabled", function(bool) {
	if (bool) {
		...
		this.icon.setAttribute("tooltiptext", "油猴脚本管理器（鼠标右键开OR关）已禁用");
		...
		this.icon.setAttribute("tooltiptext", "油猴脚本管理器（鼠标右键开OR关）已启用");
...
USL.observe = function (subject, topic, data) {
	if (topic == "xul-overlay-merged") {
		if (!USL.isready) {
			...
			USL.loadconfig();
			//dannylee
			$("showScripttoolsbutton").setAttribute("label", (this.ShowToolButton ? "油猴脚本版显示为菜单" : "油猴脚本版显示为按钮"));
			USL.toggleUI(0);
...
USL.toggleUI = function(tag) {
	...
	window.setTimeout(function() {
		...
		if (!USL.ShowToolButton) {
			...
			$("showScripttoolsbutton").setAttribute("label", "油猴脚本版显示为按钮");
		} else {
			$("UserScriptLoader-icon").appendChild($("UserScriptLoader-popup"));
			$("showScripttoolsbutton").setAttribute("label", "油猴脚本版显示为菜单");
		}
	}, 10);
};
